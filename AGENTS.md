...
agent-neutral counterpart to `CLAUDE.md`; the two files are kept identical in
substance, so update both together.

CAIVE is designed to be driven by an AI assistant in conversation with the user.
Where this document says "Claude", read it as "the assistant you are" — the
workflow expectations apply to whichever agent is doing the work.
...
├── README.md                      # User-facing docs, conversation-first
├── CLAUDE.md                      # Same guidance, Claude Code entry point
├── AGENTS.md                      # This file
├── requirements.txt
...
13. **Keep CLAUDE.md in sync with this file.** It's the Claude Code entry point; update both together.
